...
def main(args):
    ...
    if args.seed is not None:
        ...
        # 这将导致cuDNN对多个卷积算法进行测试，并选取fastest
        torch.backends.cudnn.benchmark = True
    ...
    if args.dataset == 'digits':
        ...
        if args.source == 'svhn' and args.target == 'mnist':
            ...
            # RandomCrop: 在随机的位置进行剪切，RandomRotation: 指定旋转角度
    ...
    ####################################################################################################################
    # 截止到现在，已经将数据集导入source_train_dataset, source_test_dataset, target_train_dataset, target_test_dataset
    # 并且进行了对应的变换（重新设置大小、转变为张量、【*归一化*】、...）
    # 对应设置了args.arch
    ####################################################################################################################
    ...
    #   batch_size:     每个batch加载多少个样本sample
    #   shuffle:        每一个epoch是否重新打乱顺序
    #   num_workers:    默认0，从主进程中加载；否则开启指定数量的子进程
    #   drop_last:      如果数据集不能被batch size整除，是否删除最后一个不完整的batch
    # 返回的是一个迭代器
